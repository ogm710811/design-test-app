...
import {
  ...
  authLimitComboVerify
} from '@fox/shared';

@Component({
  selector: 'fox-temp-auth-comb',
  templateUrl: './temp-auth-comb.component.html',
  styleUrls: ['./temp-auth-comb.component.css']
})

export class TempAuthCombComponent implements OnInit, AfterViewChecked  {
  ...
  async enterEventClick(): Promise<boolean> {
    ...
    this.container.mapCombos = this.tempMapComo;
    this.tableMapComboList = this.tableDataMapCombo;
    this.container.mapCombo = this.tableMapComboList;
    try {
      this.buttonStatus = ButtonStatus.WORKING;
      container = await this.operAuthCombreply(container, operDfltAuthCombOvAuthCombo, operDfltOvrdCvAuthCombo).toPromise();
      this.screen = container.operAuthCombDflt;
      this.common = container.dfhcommarea;
  ...
  private tableColumnWidth(): void {
    ...
    for (let i = 0; i < stickyCells.length; i++) {
      if (stickyCells[i].id === 'header0') {
        ...
        stickyCells[i]['style']['padding-top'] = '10px';
      }
    }
  ...
  private operAuthCombDfltServiceMainMethod(common: Dfhcommarea): Observable<Container> {
    ...
    return this.httpClient.post<Container>('/api/operator/services/operauthcombovrdservice/operauthcombovrdservice/main', JSON.stringify(common), options);
  ...
  private operAuthCombDfltServiceShowFreshMap(container: Container): Observable<Container> {
    ...
    return this.httpClient.post<Container>('/api/operator/services/operauthcombovrdservice/operauthcombovrdservice/freshmap', JSON.stringify(container), options);
  ...
  private operAuthCombDfltServiceCancelTrans(container: Container): Observable<Container> {
    ...
    return this.httpClient.post<Container>('/api/operator/services/operauthcombovrdservice/operauthcombovrdservice/returnmethod', JSON.stringify(container), options);
   ...
   */
  private operAuthCombreply(container: Container, operDfltAuthCombOvAuthCombo: OperDfltAuthCombOvAuthorityCombinationPO, operDfltOvrdCvAuthCombo: OperDfltOvrdCvAuthorityCombinationPO): Observable<Container> {
    const headers = this.opMaintenance.getHeader();
    const options = {headers: headers};
    return this.httpClient.post<Container>('/api/operator/services/operauthcombovrdservice/operauthcombovrdservice/reply', JSON.stringify(container), options);
  }

...
import {PotentialMatchesService} from './potential-matches.service';

@Component({
  selector: 'fox-potential-matches',
  templateUrl: './potential-matches.component.html',
  styleUrls: ['./potential-matches.component.less']
})
export class PotentialMatchesComponent implements OnInit {
  ...
  constructor(
    private potentialMatchSvc: PotentialMatchesService,
    ...
    this.loginState.subscribe(loggedIn => {
      console.log('REDUX LOGIN STATE => ', loggedIn);
      if (!loggedIn) {
        console.log('Clearing previous claim');
  ...
  ngOnInit(): void {
    this.potentialMatchSvc.matchesChange.subscribe((matches) => {
      ...
      this.numDisMatch = 0;
      this.displayedMatches.forEach(match => {
        if (match.matchScore) {
          this.numDisMatch++;
        }
      });

/**
 * wq-management
 * Services and orchestrations to support the management of global FOX work queues (eg. claims, documents, checks, etc).
 *
 * OpenAPI spec version: v1
 *
 *
 * NOTE: This class is auto generated by the swagger code generator program.
 * https://github.com/swagger-api/swagger-codegen.git
 * Do not edit the class manually.
 */

export interface WorkQueueItemProcessVO {
    wqiId?: number;
    userId?: string;
    processId?: number;
    status?: WorkQueueItemProcessVO.StatusEnum;
    reason?: string;
    wqiBusinessId?: string;
    businessIdType?: number;
    queue?: string;
    assignedBy?: string;
    followUpDate?: string;
    urgency?: number;
    lockedBy?: string;
}
export namespace WorkQueueItemProcessVO {
    export type StatusEnum = 'COMPLETE' | 'OPEN';
    export const StatusEnum = {
        COMPLETE: 'COMPLETE' as StatusEnum,
        OPEN: 'OPEN' as StatusEnum
    };
}
